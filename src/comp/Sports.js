...
export class Sports extends Component {
  ...
  async componentDidMount() {
    try {
      let url = "https://newsapi.org/v2/top-headlines?country=in&category=sports&apiKey=2260b7a7a2e743298e804049a4e42aa7&page=1&pageSize=12";
 ...
 next= async()=>{
  if(this.state.page>=Math.ceil(this.state.results/12)){
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=in&category=sports&apiKey=2260b7a7a2e743298e804049a4e42aa7&page=${this.state.page+1}&pageSize=12`;
        let data = await fetch(url);
        let parseddata = await data.json();
    this.setState({
